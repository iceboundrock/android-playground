    ...
    public WaterWaveAttrInit(Context context, AttributeSet attrs, int defStyle) {
        ...
        try {
            typedArray = context.obtainStyledAttributes(attrs,
                    R.styleable.WaveProgressView, defStyle, 0);
            progressWidth = typedArray.getDimensionPixelOffset(
                    R.styleable.WaveProgressView_progressWidth, 0);
            progressColor = typedArray.getColor(
                    R.styleable.WaveProgressView_progressColor, 0xFF33B5E5);
            progressBgColor = typedArray.getColor(
                    R.styleable.WaveProgressView_progressBgColor, 0xFFBEBEBE);
            waterWaveColor = typedArray.getColor(
                    R.styleable.WaveProgressView_waterWaveColor, 0XFF4BBDFE);
            waterWaveBgColor = typedArray.getColor(
                    R.styleable.WaveProgressView_waterWaveBgColor, 0xFFDDDDDD);
            progress2WaterWidth = typedArray.getDimensionPixelOffset(
                    R.styleable.WaveProgressView_progress2WaterWidth, 0);
            showProgress = typedArray.getBoolean(
                    R.styleable.WaveProgressView_showProgress, true);
            showNumerical = typedArray.getBoolean(
                    R.styleable.WaveProgressView_showNumerical, true);
            fontSize = typedArray.getDimensionPixelOffset(
                    R.styleable.WaveProgressView_fontSize, 0);
            textColor = typedArray.getColor(
                    R.styleable.WaveProgressView_txtColor, 0xFFFFFFFF);
            progress = typedArray.getInteger(
                    R.styleable.WaveProgressView_progress, 15);
            maxProgress = typedArray.getInteger(
                    R.styleable.WaveProgressView_maxProgress, 100);
        } finally {
            if (typedArray != null) {
                typedArray.recycle();
            }
